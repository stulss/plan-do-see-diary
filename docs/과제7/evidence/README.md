...
카드 1~5의 통과 기준별 상세 연결은 [`카드1-5_통과기준_스크린샷.md`](카드1-5_통과기준_스크린샷.md)에서 확인한다. 카드 1~4는 실제 화면 증거 연결을 완료했고, 카드 5의 C132 손계산 대조 화면도 2026-09-03 `T07-A17` 로 확보했다.
...
| 6 | 서로 다른 실제 날짜 5일 기록과 3일차 전 규칙 변경 | `T07-A05-five-day-rule-change-actual-pc.jpg` — DB 할 일 생성일 5일, 규칙 변경 8/31 10:30:55, 3일차 생성 15:30:23 | ◐ 날짜 순서·사용자 원래 시각 확인. 날짜별 완료·실행 기록이 DB 에 없어 지표 수치·손계산 증거는 대기 |
...
| `T07-A17-five-day-totals-hand-check-pc.jpg` | 공개 배포 돌아보기 화면, 기간 2026.08.29–2026.09.04: 계획 5·완료 5·**예상 81분·실제 20분·차이 -61분**. `5일_사용기록.md` §5 손계산(3+62+4+4+8=81, 4×5=20, 20−81=−61)과 일치 | **C132** |
...
**2026-09-03 확인**: 이 항목들은 Postman·Supabase 로그인이 필요해 AI가 대신 촬영할 수 없다(계정 비밀번호를 어떤 필드에도 대신 입력할 수 없음). 사용자가 직접 진행하는 순서와 실제 id는 `과제7_작업내역_체크리스트.md` §6에 정리했다 — `test`(id 23, task 23~27), `evidence_user_0903`(id 36, task 36).

- ~~두 계정 방향 모두에서 타 계정 할 일 GET·PATCH·DELETE 404와 목록 유출 0건을 Postman으로 촬영한다.~~
  → **2026-09-03 사용자 판단으로 종료.** 양방향 격리는 2026-09-02 공개 배포 자동 검증(`auth-isolation-results.json` 의 `isolation` 두 방향)에서 읽기·수정·삭제·날짜 이동 404, `target_unchanged` true, `leaked_rows` 0 으로 통과했다. Postman 화면은 `T07-A22`(한 방향 목록 격리)까지 남기고, 나머지 요청별 화면은 자동 검증 결과로 갈음한다.
- ~~거절 검증 전후 두 증거 할 일이 그대로임을 실제 DB 화면으로 촬영한다.~~
  → 같은 근거로 갈음한다. 자동 검증이 거절 전후 행 수(`target_rows_before`/`after`)와 불변 여부를 함께 기록한다.
- 요청 본문은 보이되 비밀번호·쿠키·토큰은 반드시 가린다.
- ~~C132의 실제 분·오차율과 손계산 대조 화면을 추가한다~~ → **2026-09-03 완료**. 5일 수치를 입력하고 `T07-A17-five-day-totals-hand-check-pc.jpg` 로 촬영했다.
