...
import { FormPending } from "@/app/form-pending";
...
async function HeaderContent() {
  ...
  return <div className="header-inner">
      ...
      {user ? <>
        ...
        <form action="/api/auth/logout" method="post" className="account-menu">
          <Link href="/account" className="account-name">{user.nickname}님</Link>
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="ko"><body>
    <FormPending />
    <header><Suspense fallback={<div className="header-inner"><Link href="/" className="brand">플랜두씨</Link></div>}><HeaderContent /></Suspense></header>
    <main>{children}</main>
  </body></html>;
}
