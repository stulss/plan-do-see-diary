...
import { DateInput, SubmitButton } from "@/app/form-controls";
...
export function CreatePlanForm({ review = "", title = "", defaultDate }: { review?: string; title?: string; defaultDate?: string }) {
  return <details className="panel create-panel" open={Boolean(review)}><summary><span><strong>새 계획 만들기</strong><small>기간과 성공 기준부터 정합니다.</small></span></summary><form className="editor-form" action="/api/plans" method="post">
    ...
    <label>시작일<DateInput name="start_date" defaultValue={defaultDate} /></label><label>종료일<DateInput name="end_date" defaultValue={defaultDate} /></label>
    ...
    <label className="wide">성공 기준<textarea name="success_criteria" required placeholder="어떤 상태가 되면 이 계획을 달성했다고 볼 수 있나요?" /></label></div><div className="editor-actions"><span>저장 후 계획 상세에서 할 일을 연결할 수 있습니다.</span><SubmitButton pendingText="저장하는 중…">계획 저장</SubmitButton></div>
  </form></details>;
}
...
export function CreateTaskForm({ plans, defaultDueDate }: { plans: PlanOption[]; defaultDueDate?: string }) {
  return <details className="panel create-panel"><summary><span><strong>새 할 일 만들기</strong><small>계획을 실행 가능한 한 단계로 나눕니다.</small></span></summary><form className="editor-form" action="/api/tasks" method="post">
    ...
    <div className="editor-grid"><label className="wide">연결할 계획 (선택)<select name="plan_id" defaultValue=""><option value="">계획 없이 추가</option>{plans.map((plan) => <option key={String(plan.id)} value={String(plan.id)}>{String(plan.title)}</option>)}</select></label>
    ...
    <label>시작일<DateInput name="start_date" defaultValue={defaultDueDate} /></label><label>마감일<DateInput name="due_date" defaultValue={defaultDueDate} /></label><label>우선순위<select name="priority" defaultValue="2"><option value="3">높음</option><option value="2">보통</option><option value="1">낮음</option></select></label><TaskTimeFields />
    <label>태그<input name="tags" placeholder="쉼표로 구분" /></label></div><div className="editor-actions"><span>계획 연결 없이도 저장할 수 있습니다.</span><SubmitButton pendingText="저장하는 중…">할 일 저장</SubmitButton></div>
  </form></details>;
}
