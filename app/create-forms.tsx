...
export function CreateTaskForm({ plans, defaultDueDate }: { plans: PlanOption[]; defaultDueDate?: string }) {
  return <details className="panel create-panel"><summary><span><strong>새 할 일 만들기</strong><small>계획을 실행 가능한 한 단계로 나눕니다.</small></span></summary><form className="editor-form" action="/api/tasks" method="post">
    ...
    <label>마감일<input type="date" name="due_date" defaultValue={defaultDueDate} /></label><label>우선순위<select name="priority" defaultValue="2"><option value="3">높음</option><option value="2">보통</option><option value="1">낮음</option></select></label><label>예상 시간(분)<input type="number" name="estimate_minutes" min="0" placeholder="0" /></label>
