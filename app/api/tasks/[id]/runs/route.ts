...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const duration = runDuration(value(body, "started_at"), value(body, "ended_at"));
    ...
    const rows = await db()`INSERT INTO run_log (task_id, started_at, ended_at, actual_minutes, blocker_reason)
      SELECT t.id, ${duration.startedAt}::timestamptz, ${duration.endedAt}::timestamptz,
        ${duration.minutes}::int, ${value(body, "blocker_reason", false) || null}::text
