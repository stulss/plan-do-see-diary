...
export default async function TaskDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="workspace-page">
    <section className="workspace-heading">
      ...
      <div className="planner-summary" aria-label="할 일 정보"><span>날짜 <strong>{task.due_date ? dateOnly(task.due_date as Date).replaceAll("-", ".") : "없음"}</strong></span><span>계획 <strong>{plannedTime}</strong></span><span><strong>{Number(task.estimate_minutes ?? 0)}</strong>분 예상</span><span><strong>{actualTotal}</strong>분 실제</span></div>
    </section>
